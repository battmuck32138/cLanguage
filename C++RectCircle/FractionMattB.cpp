...
bool FractionMattB::operator==(const FractionMattB& f1) {
	return num == f1.num && denom == f1.denom;
}

bool FractionMattB::operator>(const FractionMattB& f) {
	return (float)num / denom > (float)f.num / f.denom;
}
...
ostream& operator<< (ostream& out, const FractionMattB& f) {
    out << f.num << "/" << f.denom;// use to display denominator for fraction class

	return out;
}

istream& operator>>(istream& in, const FractionMattB& f) {
	in >> f;

	return in;
}

int FractionMattB::absVal(int input) {
	int absVal = input;
